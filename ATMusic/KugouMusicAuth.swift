...
final class KugouMusicAuth: ObservableObject {
    ...
    private let key = "atmusic.kugou.auth.v2"
    ...
    func logout() {
        ...
        updatePublishedState {
            ...
            self.avatarURL = nil
            self.vipBadge = nil
            NotificationCenter.default.post(name: .atmusicKugouLoginDidUpdate, object: nil)
        }
    }
    ...
    private static var defaultMid: String {
        #if canImport(UIKit)
        let seed = UIDevice.current.identifierForVendor?.uuidString ?? "atmusic-kugou"
        #else
        let seed = "atmusic-kugou"
        #endif
        return calculateMid(seed: seed)
    }
