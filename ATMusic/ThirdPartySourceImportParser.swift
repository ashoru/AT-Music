...
enum ThirdPartySourceImportError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .empty: return atmusicLocalized("内容为空", "Content is empty")
        case .unsupported: return atmusicLocalized("无法识别该音源格式", "Unsupported source format")
        case .invalidURL: return atmusicLocalized("链接无效", "Invalid URL")
        case .network(let message): return message
        case .noValidSource: return atmusicLocalized("没有可导入的音源", "No importable sources found")
        }
    }
}
...
enum ThirdPartySourceImportParser {
    ...
    private static func parseDictionary(_ dict: [String: Any]?, fallbackName: String?) -> ThirdPartySource? {
        ...
        let name = string(in: dict, keys: ["name", "title", "sourceName"])
            ?? fallbackName
            ?? atmusicLocalized("未命名音源", "Untitled source")
    ...
    private static func parseHeaderStyle(_ text: String, fallbackName: String?) -> [ThirdPartySource]? {
        ...
        let name = values["name"]?.isEmpty == false ? values["name"]! : (fallbackName ?? atmusicLocalized("未命名音源", "Untitled source"))
    ...
    private static func parseLooseScript(_ text: String, fallbackName: String?) -> [ThirdPartySource]? {
        ...
        let name = extractHeaderName(text) ?? fallbackName ?? atmusicLocalized("未命名脚本音源", "Untitled script source")
