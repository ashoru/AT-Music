...
enum ATMusicEqualizerPreset: String, CaseIterable, Identifiable {
    case flat
    case bass
    case vocal
    case pop
    ...
    var displayName: String {
        switch self {
        case .flat: return atmusicLocalized("默认", "Default")
        case .bass: return atmusicLocalized("低音增强", "Bass Boost")
        case .vocal: return atmusicLocalized("人声", "Vocal")
        case .pop: return atmusicLocalized("流行", "Pop")
        case .rock: return atmusicLocalized("摇滚", "Rock")
        case .classical: return atmusicLocalized("古典", "Classical")
        case .jazz: return atmusicLocalized("爵士", "Jazz")
        case .electronic: return atmusicLocalized("电子", "Electronic")
        case .custom: return atmusicLocalized("自定义", "Custom")
        }
    }
...
struct ATMusicEqualizerCustomPreset: Identifiable, Codable, Hashable {
    let id: String
    var name: String
    var gains: [Double]
...
final class ATMusicEqualizer: ObservableObject {
    static let shared = ATMusicEqualizer()
    static let settingsDidChange = Notification.Name("atmusic.equalizer.settingsDidChange")
    ...
    @Published private(set) var selectedPreset: ATMusicEqualizerPreset
    @Published private(set) var selectedCustomPresetName: String?
    @Published private(set) var preampGain: Double
    @Published private(set) var customPresets: [ATMusicEqualizerCustomPreset]
    ...
    private static let enabledKey = "atmusic.equalizer.enabled"
    private static let gainsKey = "atmusic.equalizer.bandGains"
    private static let presetKey = "atmusic.equalizer.preset"
    private static let customPresetNameKey = "atmusic.equalizer.customPresetName"
    private static let customPresetsKey = "atmusic.equalizer.customPresets"
    private static let preampKey = "atmusic.equalizer.preampGain"
    ...
    private init() {
        ...
        let storedPreset = defaults.string(forKey: Self.presetKey)
            .flatMap(ATMusicEqualizerPreset.init(rawValue:)) ?? .flat
        let storedCustomPresets = defaults.data(forKey: Self.customPresetsKey)
            .flatMap { try? JSONDecoder().decode([ATMusicEqualizerCustomPreset].self, from: $0) } ?? []
        let normalizedCustomPresets = storedCustomPresets.compactMap { preset -> ATMusicEqualizerCustomPreset? in
            let name = preset.name.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !name.isEmpty else { return nil }
            return ATMusicEqualizerCustomPreset(
                id: preset.id,
                name: name,
    ...
    func applyPreset(_ preset: ATMusicEqualizerPreset) {
    ...
    func applyCustomPreset(_ preset: ATMusicEqualizerCustomPreset) {
    ...
    func saveCustomPreset(name: String) -> Bool {
        ...
        let current = ATMusicEqualizerCustomPreset(
            name: trimmedName,
            gains: bandGains,
        ...
        if let index = customPresets.firstIndex(where: {
            ...
            customPresets[index] = ATMusicEqualizerCustomPreset(
    ...
    func deleteCustomPreset(_ preset: ATMusicEqualizerCustomPreset) {
    ...
    func makeAudioMix(for track: AVAssetTrack) -> AVAudioMix? {
        ...
        guard status == noErr, let tap else {
            ATMusicLogger.shared.log("均衡器音频处理器创建失败：\(status)", level: .error)
            return nil
        }
...
private func equalizerTapPrepare(
    ...
    let equalizer = Unmanaged<ATMusicEqualizer>.fromOpaque(storage).takeUnretainedValue()
