...
struct LogViewerSheet: View {
    @ObservedObject private var logger = ATMusicLogger.shared
    @EnvironmentObject private var theme: ThemeStore
    @Environment(\.dismiss) private var dismiss
    @AppStorage("atmusic.language") private var languageRaw = AppLanguage.chinese.rawValue
    let importedText: String?

    @State private var filter: ATMusicLogLevel? = nil
    @State private var showShare = false

    private var filtered: [ATMusicLogEntry] {
    ...
    var body: some View {
        ...
        ATMusicNavigationStack {
            ZStack {
                ...
                VStack(spacing: 0) {
                    if let importedText {
                        ScrollView {
                            Text(isEnglish ? ATMusicLogLocalizer.english(importedText) : importedText)
                        ...
                        Picker("级别", selection: $filter) {
                            Text("全部").tag(nil as ATMusicLogLevel?)
                            ForEach(ATMusicLogLevel.allCases, id: \.self) { level in
                                Text(level.rawValue).tag(level as ATMusicLogLevel?)
                            }
                        }
                                    ...
                                    ForEach(filtered) { entry in
                                        HStack(alignment: .top, spacing: 8) {
                                            Text(entry.level.rawValue)
                                                .font(ATMusicFont.appFont(9, .bold, .monospaced))
                                                .foregroundStyle(.white)
                                                .padding(.horizontal, 6)
                                                .padding(.vertical, 3)
                                                .background(Capsule().fill(entry.level.tint))
                                            VStack(alignment: .leading, spacing: 2) {
                                                Text(isEnglish ? ATMusicLogLocalizer.english(entry.message) : entry.message)
                                                    .font(ATMusicFont.appFont(12, .medium))
                                                    .foregroundStyle(Color.atmusicLabel)
                                                    .textSelection(.enabled)
                                                    .frame(maxWidth: .infinity, alignment: .leading)
                                                Text(ATMusicLogger.dateString(entry.date))
                                                    .font(ATMusicFont.appFont(9, .regular, .monospaced))
        ...
        .sheet(isPresented: $showShare) {
            ShareSheet(items: [ATMusicLogger.shared.exportLogURL()])
        }
    }
}

private enum ATMusicLogLocalizer {
