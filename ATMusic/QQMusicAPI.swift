...
final class QQMusicAPI {
    ...
    private func get(_ urlString: String, referer: String = "https://y.qq.com/", cookie: String = "") async throws -> [String: Any] {
        ...
        let startedAt = Date()
        ATMusicLogger.shared.log("QQ HTTP GET 开始 url=\(Self.sanitizedURL(urlString)) referer=\(referer) cookie=\(Self.cookieSummary(cookie))", level: .debug)
        ...
        do {
            ...
            ATMusicLogger.shared.log("QQ HTTP GET 完成 status=\(status) bytes=\(data.count) elapsed=\(Self.elapsed(startedAt))", level: .debug)
            guard let http = response as? HTTPURLResponse, http.statusCode == 200 else {
                ATMusicLogger.shared.log("QQ HTTP GET 非 200 status=\(status) response=\(Self.responseSummary(data))", level: .warn)
                throw NetEaseError.network
            }
            guard let json = parseJSON(data) else {
                ATMusicLogger.shared.log("QQ HTTP GET JSON 解析失败 response=\(Self.responseSummary(data))", level: .error)
                let snippet = String(data: data, encoding: .utf8)?.prefix(120) ?? ""
                throw NetEaseError.decoding(String(snippet))
            }
            ATMusicLogger.shared.log("QQ HTTP GET JSON 结构 \(Self.jsonSummary(json))", level: .debug)
            return json
        } catch {
            ATMusicLogger.shared.log("QQ HTTP GET 异常 elapsed=\(Self.elapsed(startedAt)) error=\(error.localizedDescription)", level: .error)
            throw error
        }
    }
    ...
    private func musicu(_ payload: [String: Any], cookie: String = "", timeout: TimeInterval = 6) async throws -> [String: Any] {
        ...
        let startedAt = Date()
        ATMusicLogger.shared.log("QQ musicu POST 开始 timeout=\(timeout)s cookie=\(Self.cookieSummary(cookie)) payload=\(Self.jsonSummary(payload))", level: .debug)
        do {
            let (data, response) = try await session.data(for: request)
            let status = (response as? HTTPURLResponse)?.statusCode ?? -1
            ATMusicLogger.shared.log("QQ musicu POST 完成 status=\(status) bytes=\(data.count) elapsed=\(Self.elapsed(startedAt))", level: .debug)
            guard let http = response as? HTTPURLResponse, http.statusCode == 200 else {
                ATMusicLogger.shared.log("QQ musicu 非 200 status=\(status) response=\(Self.responseSummary(data))", level: .warn)
                throw NetEaseError.network
            }
            guard let json = parseJSON(data) else {
                ATMusicLogger.shared.log("QQ musicu JSON 解析失败 response=\(Self.responseSummary(data))", level: .error)
                let snippet = String(data: data, encoding: .utf8)?.prefix(120) ?? ""
                throw NetEaseError.decoding(String(snippet))
            }
            ATMusicLogger.shared.log("QQ musicu JSON 结构 \(Self.jsonSummary(json))", level: .debug)
            return json
        } catch {
            ATMusicLogger.shared.log("QQ musicu 异常 elapsed=\(Self.elapsed(startedAt)) error=\(error.localizedDescription)", level: .error)
            throw error
        }
    }
    ...
    func searchPlaylists(keyword: String, limit: Int = 30) async throws -> [Playlist] {
        ...
        let paths = [
            ...
            ["req_1", "data", "body", "songlist", "list"],
            ["req_1", "data", "playlist", "list"],
            ["req_1", "data", "v_playlist", "list"],
        ]
        var rows: [[String: Any]] = []
        for path in paths {
        ...
        if rows.isEmpty {
            rows = nestedArray(json, path: ["req_1", "data", "body", "playlist"])
        }
        var seen = Set<Int>()
        return rows.prefix(max(limit, 1)).compactMap { item in
    ...
    private static func searchPlaylist(from item: [String: Any]) -> Playlist? {
        ...
        let name = (item["dissname"] as? String)
            ...
            ?? ""
        guard id > 0, !name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { return nil }
        let cover = ["logo", "diss_cover", "imgurl", "picurl", "pic_url", "cover"].lazy
            .compactMap { normalizedQQImageURL(item[$0]) }
            .first
        let count = integerValue(item["songnum"] ?? item["song_cnt"] ?? item["song_count"] ?? item["total_song_num"])
        var playlist = Playlist(id: id, name: name, coverURL: cover, trackCount: count, source: .qq)
        playlist.popularity = integerValue(item["listennum"] ?? item["listen_num"] ?? item["playnum"] ?? item["play_count"])
        playlist.creatorName = (item["creator_name"] as? String)
            ?? (item["nickname"] as? String)
            ?? (item["username"] as? String)
            ?? ""
        playlist.descriptionText = (item["desc"] as? String)
    ...
    func favoriteSongs(limit: Int = 0) async throws -> [Song] {
        ...
        ATMusicLogger.shared.log("QQ 我的喜欢加载开始 limit=\(limit <= 0 ? "无上限" : String(limit)) loggedIn=\(qqAuth.isLoggedIn) identities=[\(identities)] playlistUin=\(Self.maskedIdentity(qqAuth.playlistUin)) gtk=已计算", level: .info)
        guard qqAuth.isLoggedIn else {
            ATMusicLogger.shared.log("QQ 我的喜欢终止：当前未登录", level: .error)
            return []
        }
        ...
        if let favJson = try? await get(legacyFavURL, referer: "https://y.qq.com/n/yqq/playlist", cookie: qqAuth.cookieHeader),
           let mapid = Self.likedMapID(favJson), mapid > 0 {
            ATMusicLogger.shared.log("QQ 我的喜欢旧接口成功 mapid=\(mapid) json=\(Self.jsonSummary(favJson))", level: .info)
            ...
            if let detailJson = try? await get(detailURL, referer: "https://y.qq.com/", cookie: qqAuth.cookieHeader),
               ...
               let rawSongs = cdlist.first?["songlist"] as? [[String: Any]] {
                ...
                ATMusicLogger.shared.log("QQ 我的喜欢旧详情 mapid=\(mapid) cdlist=\(cdlist.count) rawSongs=\(rawSongs.count) parsedSongs=\(songs.count)", level: songs.isEmpty ? .warn : .info)
                if !songs.isEmpty && (limit > 0 || rawSongs.count < pageSize) { return songs }
            } else {
                ATMusicLogger.shared.log("QQ 我的喜欢旧详情未匹配 cdlist/songlist mapid=\(mapid)", level: .warn)
            }
        } else {
            ATMusicLogger.shared.log("QQ 我的喜欢旧接口未得到有效 mapid", level: .warn)
        }
        ...
        for cookie in cookieCandidates {
            ...
            while allSongs.count < targetLimit {
                ...
                ATMusicLogger.shared.log("QQ 我的喜欢 CgiGetDiss 分页 begin=\(begin) raw=\(rawPage.count) parsed=\(pageSongs.count) new=\(newSongs.count) total=\(allSongs.count)", level: pageSongs.isEmpty ? .warn : .debug)
        ...
        for identity in identityCandidates {
            ...
            for cookie in cookieCandidates {
                ...
                ATMusicLogger.shared.log("QQ 我的喜欢带身份接口 identity=\(identity) raw=\(Self.favoriteSongArray(from: favJson).count) parsed=\(songs.count)", level: songs.isEmpty ? .warn : .info)
                if !songs.isEmpty { return songs }
            }
        }

        guard let mapid = await likedPlaylistID(qqAuth: qqAuth, cookies: cookieCandidates), mapid > 0 else {
            ATMusicLogger.shared.log("QQ 我的喜欢歌单解析失败：未找到真实歌单 ID", level: .error)
            return []
        }
        ...
        for cookie in cookieCandidates {
            ...
            ATMusicLogger.shared.log("QQ 我的喜欢最终详情 mapid=\(mapid) songs=\(songs.count)", level: songs.isEmpty ? .warn : .info)
            if !songs.isEmpty { return songs }
        }
        let songs = try await playlistSongs(listID: mapid, preferredCookie: nil, limit: fallbackLimit)
        ATMusicLogger.shared.log("QQ 我的喜欢加载结束 mapid=\(mapid) songs=\(songs.count)", level: songs.isEmpty ? .error : .info)
        return songs
    }
    ...
    private func resolveMediaMid(songmid: String, provided: String?, qqAuth: QQMusicAuth) async -> String {
        ...
        guard let json = try? await musicu(payload, cookie: qqAuth.isLoggedIn ? qqAuth.cookieHeader : ""),
            ...
            ATMusicLogger.shared.log("QQ 歌曲详情未返回 media_mid，回退 songmid", level: .debug)
            return songmid
        }
        ATMusicLogger.shared.log("QQ 歌曲详情：media_mid=已获取 是否不同=\(mediaMid == songmid ? "否" : "是")", level: .debug)
        return mediaMid
    }
    ...
    private func vkeyURL(songmid: String, mediaMid: String?, br: String, uin: String, loginKey: String, guid: String, qqAuth: QQMusicAuth) async throws -> String? {
        ...
        guard !playableInfos.isEmpty else {
            ...
            ATMusicLogger.shared.log("QQ vkey 无可播地址：音质=\(br) result=\(result) 已登录=\(qqAuth.isLoggedIn ? "是" : "否")", level: .debug)
            return nil
        }
        ...
        for info in playableInfos {
            ...
            for candidate in candidateURLs {
                ...
                case .playable:
                    ...
                    ATMusicLogger.shared.log("QQ 音频地址验证成功：音质=\(br) 文件=\(filename)", level: .debug)
                    return candidate
                case .forbidden:
                    continue
                case .indeterminate:
        ...
        if let unverifiedCandidate {
            ATMusicLogger.shared.log("QQ CDN 拒绝预探测，保留地址交给 AVPlayer：音质=\(br)", level: .debug)
            return unverifiedCandidate
        }
        ATMusicLogger.shared.log("QQ vkey 返回地址但 CDN 验证失败：音质=\(br) 候选=\(playableInfos.count)", level: .debug)
        return nil
    }
    ...
    private static var deviceGuid: String {
        let key = "atmusic.qqmusic.guid.v1"
    ...
    private static func qqBR(for quality: ATMusicAudioQuality) -> String {
        switch quality {
    ...
    func comments(songID: Int, limit: Int = 25, pagenum: Int = 0) async throws -> QQCommentPage {
        ...
        let normalComments = normal.compactMap { parse($0, isHot: false) }
        return QQCommentPage(hot: hotComments, comments: normalComments, total: commentTotal)
    }
    ...
    func userPlaylists(uin: String) async throws -> [Playlist] {
        let qqAuth = QQMusicAuth.shared
        ATMusicLogger.shared.log("QQ 歌单列表加载开始 requestedUin=\(Self.maskedIdentity(uin)) playlistUin=\(Self.maskedIdentity(qqAuth.playlistUin)) identities=\(qqAuth.playlistIdentityCandidates.map(Self.maskedIdentity).joined(separator: ",")) cookies=\(cookieCandidatesSummary(qqAuth))", level: .info)
        ...
        if playlists.isEmpty {
            ATMusicLogger.shared.log("QQ 歌单列表为空（uin=\(uin)，playlistUin=\(requestUin)），可能是微信 Cookie 未包含完整登录态或接口返回空", level: .error)
        }
        ...
        ATMusicLogger.shared.log("QQ 歌单列表加载结束 count=\(playlists.count) likedPresent=\(playlists.contains { $0.id == Self.qqLikedPlaylistID }) names=\(playlists.map(\.name).joined(separator: " | "))", level: playlists.isEmpty ? .error : .info)
    ...
    private static func hotRecommendPlaylist(_ item: [String: Any]) -> Playlist? {
        ...
        // `listennum/listen_num` 是播放次数，不是歌曲数量。
        // 首页“xx 首”只能来自真实歌曲数字段。
        let count = integerValue(item["songnum"] ?? item["song_cnt"] ?? item["song_count"] ?? item["track_count"])
    ...
    func hotPlaylists(limit: Int = 18) async throws -> [Playlist] {
        ...
        request.setValue("https://y.qq.com/", forHTTPHeaderField: "Referer")
        let startedAt = Date()
        ATMusicLogger.shared.log("QQ 官网热门歌单加载开始 limit=\(limit)", level: .info)
        let (data, response) = try await session.data(for: request)
        let status = (response as? HTTPURLResponse)?.statusCode ?? -1
        guard status == 200, let html = String(data: data, encoding: .utf8) else {
            ATMusicLogger.shared.log("QQ 官网热门歌单加载失败 status=\(status) bytes=\(data.count)", level: .error)
            throw NetEaseError.network
        }
        ...
        if let initialData = Self.initialData(from: html) {
            ...
            let playlists = Self.hotRecommendArray(from: initialData)
                ...
                .prefix(max(1, limit))
            if !playlists.isEmpty {
                ATMusicLogger.shared.log("QQ 官网热门歌单 SSR 解析完成 count=\(playlists.count) elapsed=\(Self.elapsed(startedAt))", level: .info)
                return await enrichHotPlaylistMetadata(Array(playlists))
            }
            ATMusicLogger.shared.log("QQ 官网热门歌单 SSR 存在但 hotRecommend 为空，尝试正则兜底", level: .warn)
        } else {
            ATMusicLogger.shared.log("QQ 官网热门歌单未找到 INITIAL_DATA，尝试正则兜底", level: .warn)
        }
        ...
        for match in regex.matches(in: html, range: range) {
            ...
            _ = countRange // 正则捕获的是 listennum，仅用于兼容定位，不能作为歌曲数量。
            guard !name.isEmpty else { continue }
            seen.insert(id)
            playlists.append(Playlist(id: id, name: name, coverURL: Self.normalizedQQImageURL(image), trackCount: 0, source: .qq))
            if playlists.count >= max(1, limit) { break }
        }
        ATMusicLogger.shared.log("QQ 官网热门歌单加载完成 count=\(playlists.count) elapsed=\(Self.elapsed(startedAt))", level: playlists.isEmpty ? .warn : .info)
        if !playlists.isEmpty { return await enrichHotPlaylistMetadata(playlists) }
        return try await recommendPlaylistsFallback(limit: limit)
    }

    /// 官网 SSR 往往只给播放次数/占位封面。QQ 的详情接口在批量传多个 disstid 时
    /// 会把 songnum 返回为 0，因此这里必须逐歌单请求；每批最多并发 6 个，兼顾准确性与速度。
    private func enrichHotPlaylistMetadata(_ playlists: [Playlist]) async -> [Playlist] {
        guard !playlists.isEmpty else { return playlists }

        var metadata: [Int: (cover: URL?, count: Int)] = [:]
        let chunkSize = 6
        var offset = 0
        while offset < playlists.count {
            let chunk = Array(playlists[offset..<min(offset + chunkSize, playlists.count)])
            await withTaskGroup(of: (Int, URL?, Int)?.self) { group in
                for playlist in chunk {
                    group.addTask { [weak self] in
                        guard let self else { return nil }
                        let id = playlist.id
                        let url = "https://c.y.qq.com/qzone/fcg-bin/fcg_ucc_getcdinfo_byids_cp.fcg?type=1&json=1&utf8=1&onlysong=0&new_format=1&disstid=\(id)&loginUin=0&hostUin=0&format=json&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq.json&needNewCode=0"
                        guard let json = try? await self.get(url, referer: "https://y.qq.com/"),
                              let item = (json["cdlist"] as? [[String: Any]])?.first else {
                            return (id, nil, 0)
                        }
                        let cover = Self.normalizedQQImageURL(item["logo"])
                            ?? Self.normalizedQQImageURL(item["coveradurl"])
                        let explicitCount = Self.integerValue(
                            item["songnum"] ?? item["song_num"] ?? item["song_count"]
                                ?? item["total_song_num"] ?? item["track_count"]
                        )
                        let actualListCount = (item["songlist"] as? [[String: Any]])?.count ?? 0
                        let count = explicitCount > 0 ? explicitCount : actualListCount
                        return (id, cover, count)
                    }
                }
                for await result in group {
                    guard let result else { continue }
                    metadata[result.0] = (result.1, result.2)
                }
            }
            offset += chunkSize
        }
        ...
        let result = playlists.map { playlist -> Playlist in
            guard let meta = metadata[playlist.id] else { return playlist }
            let cover = meta.cover ?? playlist.coverURL
            let count = meta.count > 0 ? meta.count : playlist.trackCount
            return Playlist(
                id: playlist.id,
                name: playlist.name,
                coverURL: cover,
        ...
        let resolvedCounts = metadata.values.filter { $0.count > 0 }.count
        ATMusicLogger.shared.log(
            "QQ 热门歌单元数据补全完成 requested=\(playlists.count) resolvedCounts=\(resolvedCounts)",
            level: .info
        )
        return result
    }
    ...
    private func playlistSongs(listID: Int, preferredCookie: String?, limit: Int) async throws -> [Song] {
        let pageSize = 300
        let targetLimit = limit > 0 ? limit : Int.max
        ATMusicLogger.shared.log("QQ 歌单歌曲加载开始 listID=\(listID) limit=\(limit > 0 ? String(limit) : "无上限") preferredCookie=\(preferredCookie == nil ? "无" : "有")", level: .info)
    ...
    /// QQ 歌手全部歌曲（分页加载，避免接口单页最多返回 30 首）。
    func artistHotSongs(mid: String?, name: String, limit: Int = 5_000) async throws -> [Song] {
        ...
        while songs.count < targetCount {
            ...
            begin += list.count
            // QQ 兼容接口可能返回动态短页，继续按实际返回数量翻页，
            // 由空页或重复页结束，避免歌手歌曲只显示首批。
        }
        return Array(songs.prefix(targetCount))
    }

    /// QQ 歌手全部专辑（fcg_v8_singer_album 分页读取；接口异常时返回空）
    func artistAlbums(mid: String?, name: String, limit: Int = 5_000) async throws -> [Album] {
        guard let mid, !mid.isEmpty else { return [] }
        let targetCount = max(limit, 1)
        let pageSize = min(targetCount, 50)
        var albums: [Album] = []
        var seen = Set<String>()
        var begin = 0
        while albums.count < targetCount {
            let url = "https://c.y.qq.com/v8/fcg-bin/fcg_v8_singer_album.fcg?singer_mid=\(mid)&order=time&begin=\(begin)&num=\(pageSize)&format=json"
            guard let json = try? await get(url) else { break }
            let data = json["data"] as? [String: Any] ?? [:]
            let list = data["list"] as? [[String: Any]] ?? []
            guard !list.isEmpty else { break }
            ...
            for item in list {
                ...
                let id = "qq-album-\(albumMid)-\(albumName)"
                ...
                added += 1
                if albums.count >= targetCount { break }
            }
            guard added > 0 else { break }
            begin += list.count
            if list.count < pageSize { break }
        }
        return Array(albums.prefix(targetCount))
    }
