...
final class NetEaseAPI {
    ...
    private let cookiesKey = "atmusic.netease.cookies"
    ...
    /// 歌单搜索接口经常只返回标题和封面；详情接口补齐作者、简介与播放量，
    /// 供歌单页头部预览使用。
    func playlistMetadata(id: Int) async throws -> PlaylistMetadata {
        let json = try await request("/api/v6/playlist/detail", payload: ["id": id, "n": 0, "s": 8], crypto: "eapi")
        let playlist = json["playlist"] as? [String: Any] ?? [:]
        let creator = (playlist["creator"] as? [String: Any])?["nickname"] as? String
        let description = (playlist["description"] as? String)
            ?? (playlist["copywriter"] as? String)
        let popularity = playlist["playCount"] as? Int ?? playlist["subscribedCount"] as? Int
        return PlaylistMetadata(creatorName: creator, descriptionText: description, popularity: popularity)
    }
    ...
    /// 搜索歌单（type=1000）。歌单搜索不能复用本地歌单列表，否则聚合搜索永远只会看到已登录账号的歌单。
    func searchPlaylists(keyword: String, limit: Int = 30, offset: Int = 0) async throws -> [Playlist] {
        ...
        return list.compactMap { item in
            guard var playlist = Playlist(json: item) else { return nil }
            playlist.popularity = item["playCount"] as? Int ?? item["subscribedCount"] as? Int
            playlist.descriptionText = (item["description"] as? String) ?? playlist.descriptionText
            return playlist
        }
    }
    ...
    /// 歌手全部歌曲（分页加载，接口单页最多返回 30 首）。
    func artistHotSongs(artistID: Int, limit: Int = 5_000) async throws -> [Song] {
        ...
        while songs.count < targetCount {
            ...
            offset += list.count
            // 部分兼容接口会返回不足 pageSize 的短页，但后面仍有数据；
            // 只有空页或重复页才结束，避免歌手歌曲被截断。
        }
        return Array(songs.prefix(targetCount))
    }

    /// 歌手全部专辑。网易云接口按 offset 分页，不能只读取 hotAlbums 的第一页。
    func artistAlbums(artistID: Int, limit: Int = 5_000) async throws -> [Album] {
        let targetCount = max(limit, 1)
        ...
        while albums.count < targetCount {
            let json = try await request("/api/artist/albums", payload: ["id": artistID, "limit": pageSize, "offset": offset], crypto: "weapi")
            let list = json["hotAlbums"] as? [[String: Any]] ?? []
            guard !list.isEmpty else { break }
            var added = 0
            for item in list {
                guard let id = item["id"] as? Int, seen.insert("netease-\(id)").inserted else { continue }
    ...
    func like(id: Int, liked: Bool) async throws -> Bool {
        ...
        ATMusicLogger.shared.log("网易云红心同步失败：id=\(id) liked=\(liked) code=\(lastCode) \(lastError)", level: .error)
        return false
    }
    ...
    func playlistSquare(cat: String = "全部", order: String = "hot", limit: Int = 12, offset: Int = 0) async throws -> [Playlist] {
        let json = try await request("/api/playlist/list", payload: ["cat": cat, "order": order, "limit": limit, "offset": offset, "total": true], crypto: "weapi")
    ...
    func highQualityPlaylists(cat: String = "全部", limit: Int = 18, offset: Int = 0) async throws -> [Playlist] {
        let json = try await request("/api/playlist/highquality/list", payload: ["cat": cat, "limit": limit, "offset": offset, "total": true], crypto: "weapi")
        let list = json["playlists"] as? [[String: Any]] ?? []
        return list.compactMap(Playlist.init(json:))
    }

    /// 官方歌单分类（官网 discover/playlist 的全部子分类；失败返回空数组）
    func playlistCatlist() async -> [String] {
        guard let json = try? await request("/api/playlist/catlist", payload: [:], crypto: "weapi") else { return [] }
        let top = json["categories"] as? [String: Any] ?? [:]
        let topNames = top.values.compactMap { $0 as? String }
        let sub = json["sub"] as? [[String: Any]] ?? []
        let subNames = sub.compactMap { $0["name"] as? String }
        return topNames + subNames
    }
