...
final class KugouMusicAPI {
    ...
    func userPlaylists() async throws -> [Playlist] {
        ...
        ATMusicLogger.shared.log("酷狗歌单同步：status=\(status) code=\(code) 返回 \(raw.count) 个", level: .debug)
        var seen = Set<Int>()
    ...
    func personalFM(limit: Int = 12) async throws -> [Song] {
        ...
        for _ in 0..<batchCount {
            ...
            if batch.isEmpty {
                ...
                ATMusicLogger.shared.log("酷狗私人漫游接口无歌曲：code=\(code) message=\(message.isEmpty ? "无" : message)", level: .debug)
                break
            }
        ...
        if songs.isEmpty,
           ...
           !fallback.isEmpty {
            ATMusicLogger.shared.log("酷狗私人漫游无个性化结果，使用每日推荐兜底：返回 \(fallback.count) 首", level: .debug)
            return Array(fallback.prefix(target))
        }
        ATMusicLogger.shared.log("酷狗私人漫游：返回 \(songs.count) 首", level: songs.isEmpty ? .warn : .info)
        return songs
    }
    ...
    func searchSongs(keyword: String, limit: Int = 30) async throws -> [Song] {
        if let upstream = try? await upstreamSearchSongs(keyword: keyword, limit: limit), !upstream.isEmpty {
            ATMusicLogger.shared.log("酷狗 v3 搜索完成：\(keyword) 结果=\(upstream.count)", level: .info)
            return upstream
        }
        if let complete = try? await searchSongsComplete(keyword: keyword, limit: limit), !complete.isEmpty {
            ATMusicLogger.shared.log("酷狗综合搜索完成：\(keyword) 结果=\(complete.count)", level: .info)
            return complete
        }
        ...
        ATMusicLogger.shared.log("酷狗搜索完成：\(keyword) 结果=\(songs.count)", level: .info)
        return songs
    }
    ...
    func searchPlaylists(keyword: String, limit: Int = 30) async throws -> [Playlist] {
        ...
        guard let url = components.url else {
            throw NetEaseError.unknown("酷狗歌单搜索地址无效")
        }
        let json = try await getJSON(url, ua: Self.browserUA)
        let rows = Self.deepArrays(json, names: ["info", "special", "special_list", "list", "data"])
        var seen = Set<Int>()
        return rows.compactMap { item in
            ...
            updated.popularity = Self.int(item["playcount"] ?? item["play_count"] ?? item["listencount"] ?? item["heat"])
            return updated
        }
    }
    ...
    func searchArtists(keyword: String, limit: Int = 40) async throws -> [Artist] {
        if let artists = try? await upstreamSearchArtists(keyword: keyword, limit: limit), !artists.isEmpty {
            ATMusicLogger.shared.log("酷狗歌手搜索完成：\(keyword) 结果=\(artists.count)", level: .info)
            return artists
        }
    ...
    func artistSongs(authorID: String, page: Int = 1, limit: Int = 100) async throws -> [Song] {
        ...
        ATMusicLogger.shared.log("酷狗歌手歌曲：author=\(authorID) page=\(page) 返回 \(songs.count) 首", level: .debug)
    ...
    private func officialArtistSongs(authorID: String, page: Int, limit: Int) async throws -> [Song] {
        ...
        if !songs.isEmpty {
            ATMusicLogger.shared.log("酷狗 singer/song：author=\(authorID) page=\(page) 返回 \(songs.count) 首", level: .debug)
        }
        return songs
    }
    ...
    func topLists(limit: Int = 10) async throws -> [KugouTopInfo] {
        if let upstream = try? await upstreamTopLists(limit: limit), !upstream.isEmpty {
            ATMusicLogger.shared.log("酷狗 v6 排行榜列表：返回 \(upstream.count) 个", level: .debug)
            return upstream
        }
    ...
    func rankSongs(rankID: Int, limit: Int = 100) async throws -> [Song] {
        ...
        if !upstream.isEmpty {
            ATMusicLogger.shared.log("酷狗 KMR 排行榜歌曲：rankid=\(rankID) 返回 \(upstream.count) 首", level: .debug)
        }
        ...
        if merged.count < target,
           ...
           !official.isEmpty {
            ...
            ATMusicLogger.shared.log("酷狗排行榜歌曲补齐：rankid=\(rankID) KMR=\(upstream.count) 官网=\(official.count) 当前=\(merged.count)", level: .debug)
        }
        ...
        if merged.count < target,
           ...
           !mobile.isEmpty {
            ...
            ATMusicLogger.shared.log("酷狗排行榜歌曲补齐：rankid=\(rankID) 当前=\(merged.count) 移动端=\(mobile.count)", level: .debug)
        }
    ...
    private func officialWebTopLists(limit: Int) async throws -> [KugouTopInfo] {
        ...
        ATMusicLogger.shared.log("酷狗官网热门榜单：返回 \(rows.count) 个", level: .debug)
    ...
    private func officialWebRankSongs(rankID: Int, limit: Int) async throws -> [Song] {
        ...
        ATMusicLogger.shared.log("酷狗官网排行榜歌曲：rankid=\(rankID) 返回 \(songs.count) 首", level: .debug)
        return songs
    }
    ...
    private func officialWebPlaylists(limit: Int) async throws -> [Playlist] {
        ...
        ATMusicLogger.shared.log("酷狗官网歌单广场：返回 \(rows.count) 个", level: .debug)
        return Array(rows.prefix(limit))
    }
    ...
    private func officialWebPlaylistSongs(listID: Int) async throws -> [Song] {
        ...
        let songs = rows.compactMap(Self.mapCompleteTrack)
        ATMusicLogger.shared.log("酷狗官网歌单歌曲：specialid=\(listID) 返回 \(songs.count) 首", level: .debug)
        return songs
    }
    ...
    func songURL(song: Song, quality: ATMusicAudioQuality? = nil) async throws -> String? {
        await refreshMembershipStatusIfNeeded()
        let requestedQuality = quality ?? ATMusicAudioQuality.current
        ...
        if Self.qualityHashCandidates(primary: primary, qualityHashes: qualityHashes, quality: requestedQuality).isEmpty,
           let completed = try? await completePlaybackMetadata(for: song) {
            ...
            ATMusicLogger.shared.log("酷狗播放元数据补齐：\(song.name) hash=\((primary ?? "").isEmpty ? "无" : "有") albumAudioId=\(albumAudioId ?? "")", level: .debug)
        }
    ...
    func songURL(hash: String, albumAudioId: String?, albumId: String?) async throws -> String? {
        try await songURL(hashes: [hash], albumAudioId: albumAudioId, albumId: albumId, quality: ATMusicAudioQuality.current)
    }

    private func songURL(hashes: [String], albumAudioId: String?, albumId: String?, quality: ATMusicAudioQuality) async throws -> String? {
        let auth = KugouMusicAuth.shared
        ...
        for hash in hashes {
            if let latest = try? await upstreamSongURLOnce(
                ...
                ATMusicLogger.shared.log("酷狗 KuGouMusicApi 播放地址命中：hash=\(hash.prefix(8))", level: .debug)
                return url
            }
            for vipType in vipTypes {
                ...
                if let url = result.url, !url.isEmpty {
                    if vipType != auth.vipType {
                        ATMusicLogger.shared.log("酷狗播放地址命中：hash=\(hash.prefix(8)) vipType=\(vipType)", level: .debug)
                    }
                    return url
                }
            }
            ...
            for vipType in vipTypes {
                ...
                if let url = v5.url, !url.isEmpty {
                    if vipType != auth.vipType {
                        ATMusicLogger.shared.log("酷狗 v5 播放地址命中：hash=\(hash.prefix(8)) vipType=\(vipType)", level: .debug)
                    }
                    return url
                }
            }
        ...
        ATMusicLogger.shared.log("酷狗播放地址为空：hash候选=\(hashes.count) vipType=\(vipTypeText) 已登录=\(auth.isLoggedIn ? "是" : "否") token=\(auth.token.isEmpty ? "无" : "有") dfid=\(auth.dfid.isEmpty ? "无" : "有") status=\(lastStatus) code=\(lastCode)", level: .debug)
        return nil
    }
    ...
    private func upstreamSongURLOnce(
        ...
        albumAudioId: String?,
        albumId: String?,
        quality requestedQuality: ATMusicAudioQuality
    ...
    private func songURLV5Once(hash: String, albumAudioId: String?, albumId: String?, vipType: Int, quality requestedQuality: ATMusicAudioQuality) async throws -> (url: String?, status: Int, code: Int) {
        let auth = KugouMusicAuth.shared
        ...
        for listID in ["3", "2"] {
            do {
                ...
                if let url = probe.url, !url.isEmpty {
                    ...
                    ATMusicLogger.shared.log("酷狗会员状态补齐：播放探测成功 listid=\(listID) vipType=1", level: .debug)
                    return
                }
                ATMusicLogger.shared.log("酷狗会员状态探测未命中：listid=\(listID) status=\(probe.status) code=\(probe.code)", level: .debug)
            } catch {
                ATMusicLogger.shared.log("酷狗会员状态探测失败：listid=\(listID) \(error.localizedDescription)", level: .debug)
            }
        }
    }
    ...
    func comments(mixSongID: String, hash: String?, page: Int = 1, limit: Int = 30) async throws -> KugouCommentPage {
        ...
        do {
            ...
            ATMusicLogger.shared.log("酷狗评论 POST 失败：\(error.localizedDescription)，尝试 GET 兜底", level: .debug)
            do {
                json = try await gatewayCommentJSON(params: params)
            } catch {
                ATMusicLogger.shared.log("酷狗评论 gateway 全部失败：\(error.localizedDescription)，尝试旧版评论接口", level: .debug)
    ...
    private func gatewayCommentJSON(params: [String: String]) async throws -> [String: Any] {
        do {
            ...
            ATMusicLogger.shared.log("酷狗评论 GET 失败：\(error.localizedDescription)，尝试备用路由", level: .debug)
            let response = try await gatewayRequest(
    ...
    private static func parseComments(json: [String: Any], page: Int, songName: String) -> KugouCommentPage {
        ...
        let comments = rows.compactMap { raw -> SongComment? in
            ...
            let hotFlag = Self.int(raw["is_hot"] ?? raw["isHot"] ?? raw["is_top"] ?? raw["hot"])
            return SongComment(
                id: id,
                content: content,
        ...
        ATMusicLogger.shared.log("酷狗评论：mixsongid=\(songName) page=\(page) 返回 \(comments.count) 条", level: .debug)
        return KugouCommentPage(
            hot: comments.filter(\.isHot),
            comments: comments.filter { !$0.isHot },
            total: total
        )
    }
    ...
    private func registerDevice() async {
        ...
        do {
            ...
            ATMusicLogger.shared.log("酷狗设备注册：dfid=\(dfid.isEmpty ? "未返回" : "已获取")", level: .debug)
        } catch {
            ATMusicLogger.shared.log("酷狗设备注册失败：\(error.localizedDescription)", level: .debug)
        }
    }
    ...
    private static func mapPlaylist(_ raw: [String: Any]) -> Playlist? {
        ...
        let cover = string(raw["pic"] ?? raw["img"] ?? raw["imgurl"] ?? raw["cover"] ?? raw["sizable_cover"] ?? raw["list_pic"]).replacingOccurrences(of: "{size}", with: "240")
        let count = int(raw["count"] ?? raw["song_count"] ?? raw["total"] ?? raw["file_count"] ?? raw["songcount"])
        var playlist = Playlist(id: id, name: name.isEmpty ? "酷狗歌单" : name, coverURL: URL(string: cover), trackCount: count, source: .kugou)
        playlist.creatorName = string(raw["creator_name"] ?? raw["nickname"] ?? raw["username"] ?? raw["author_name"])
        playlist.descriptionText = string(raw["description"] ?? raw["desc"] ?? raw["intro"])
        playlist.popularity = int(raw["playcount"] ?? raw["play_count"] ?? raw["listencount"] ?? raw["heat"])
        return playlist
    }
    ...
    private static func qualityHashCandidates(primary: String?, qualityHashes: [String: String]?, quality requestedQuality: ATMusicAudioQuality? = nil) -> [String] {
        let requested = requestedQuality ?? ATMusicAudioQuality.current
        let order: [String]
        switch requested {
        case .hires:
