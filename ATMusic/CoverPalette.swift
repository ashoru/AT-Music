...
struct CoverPalette {
    ...
    static func fallback(colorScheme: ColorScheme) -> CoverPalette {
        let accent = Color.atmusicHighlight
        return CoverPalette(
            backgroundTop: Color(uiColor: .atmusicBackground),
            backgroundBottom: Color(uiColor: .atmusicBackground).opacity(0.86),
            accent: accent,
            accentSoft: accent.opacity(0.28),
            text: Color(uiColor: .atmusicLabel),
            secondary: Color(uiColor: .atmusicSecondary),
            glassTint: accent
        )
    }
