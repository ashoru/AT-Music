...
@objc protocol ATMusicLXScriptBufferExports: JSExport {
    func from(_ text: String, _ encoding: String) -> NSDictionary
    func bufToString(_ buffer: JSValue?, _ encoding: String) -> String
}

@objc protocol ATMusicLXScriptUtilsExports: JSExport {
    var buffer: ATMusicLXScriptBufferBridge { get }
    var crypto: ATMusicLXScriptCryptoBridge { get }
}

@objc protocol ATMusicLXScriptCryptoExports: JSExport {
    func md5(_ value: String) -> String
    func aesEncrypt(_ data: String, _ mode: String, _ key: String, _ iv: String) -> NSDictionary
}

@objc protocol ATMusicLXScriptBridgeExports: JSExport {
    ...
    var utils: ATMusicLXScriptUtilsBridge { get }
    var version: String { get }
    var currentScriptInfo: NSDictionary { get }
}

@objc protocol ATMusicLXScriptDoneExports: JSExport {
    func resolve(_ value: JSValue?)
    func reject(_ value: JSValue?)
}

final class ATMusicLXScriptBufferBridge: NSObject, ATMusicLXScriptBufferExports {
    func from(_ text: String, _ encoding: String) -> NSDictionary {
...
final class ATMusicLXScriptUtilsBridge: NSObject, ATMusicLXScriptUtilsExports {
    let buffer = ATMusicLXScriptBufferBridge()
    let crypto = ATMusicLXScriptCryptoBridge()
}

final class ATMusicLXScriptCryptoBridge: NSObject, ATMusicLXScriptCryptoExports {
    func md5(_ value: String) -> String {
    ...
    func aesEncrypt(_ data: String, _ mode: String, _ key: String, _ iv: String) -> NSDictionary {
        ...
        guard status == kCCSuccess else {
            return ATMusicLXScriptBufferBridge.dictionary(for: Data())
        }
        return ATMusicLXScriptBufferBridge.dictionary(for: Data(output.prefix(outputLength)))
    }
...
final class ATMusicLXScriptDoneBridge: NSObject, ATMusicLXScriptDoneExports {
...
final class ATMusicLXScriptBridge: NSObject, ATMusicLXScriptBridgeExports {
    private let runtimeQueue: DispatchQueue
    private let session: URLSession
    weak var runtime: ATMusicLXScriptRuntime?
    ...
    let utils = ATMusicLXScriptUtilsBridge()
    let version: String
    ...
    func request(_ url: String, _ options: JSValue?, _ callback: JSValue?) {
        ...
        if let body = requestOptions["body"] {
            if let bodyData = body as? Data {
                ...
                if dict["__atmusicBuffer"] as? Bool == true,
...
final class ATMusicLXScriptRuntime {
    private let sourceID: String
    private let queue = DispatchQueue(label: "AT Music.LXScriptRuntime")
    private let context: JSContext
    private let bridge: ATMusicLXScriptBridge
    ...
    init?(source: ThirdPartySource, script: String) {
        ...
        self.bridge = ATMusicLXScriptBridge(runtimeQueue: queue, source: source, script: script)
        self.bridge.runtime = self

        context.exceptionHandler = { _, exception in
            if let exception {
                ATMusicLogger.shared.log("第三方脚本执行异常：\(exception)", level: .debug)
            }
        }
        ...
        queue.sync {
            ...
            (function() {
                ...
                if (typeof globalThis.Buffer === 'undefined') {
                    var makeATMusicBuffer = function(value, encoding) {
                        if (value && value.__atmusicBuffer) {
                            return value;
                        }
                    ...
                    globalThis.Buffer = {
                        from: makeATMusicBuffer,
                        isBuffer: function(value) {
                            return !!(value && value.__atmusicBuffer);
                        }
                    };
                }
                ...
                if (!JSON.__atmusicOriginalParse) {
                    JSON.__atmusicOriginalParse = JSON.parse;
                    JSON.parse = function(value) {
                        return value !== null && typeof value === 'object'
                            ? value
                            : JSON.__atmusicOriginalParse(value);
                    };
                }
            })();
            """)
            ...
            if context.exception == nil {
                _ = context.evaluateScript("globalThis.__atmusicPlugin = module.exports;")
                _ = context.evaluateScript("globalThis.__atmusicMusicPlugin = globalThis.MusicPlugin || {};")
    ...
    func receive(event: String, payload: Any?) {
        ...
        if let dict = payload as? [String: Any], let sources = dict["sources"] as? [String: Any] {
            ATMusicLogger.shared.log("第三方脚本初始化：\(sourceID) sources=\(sources.keys.sorted().joined(separator: ","))", level: .debug)
            ...
            ATMusicLogger.shared.log("第三方脚本初始化：\(sourceID) sources=\(names)", level: .debug)
        }
    }
    ...
    func invokeRequest(payload: [String: Any], timeout: TimeInterval = 12) -> Any? {
        ...
        let done = ATMusicLXScriptDoneBridge()
        ...
        queue.async {
            ...
            (function() {
                if (typeof __atmusicPlugin !== 'undefined' && typeof __atmusicPlugin.musicUrl === 'function') {
                    return 'musicUrl';
                }
                if (typeof __atmusicMusicPlugin !== 'undefined' && typeof __atmusicMusicPlugin.getMusicUrl === 'function') {
                    return 'musicPluginGetMusicUrl';
                }
                if (typeof __atmusicPlugin !== 'undefined' && typeof __atmusicPlugin.getMusicUrl === 'function') {
            ...
            guard handler != nil || !resolverKind.isEmpty else {
                ATMusicLogger.shared.log("第三方脚本没有可用的音乐地址解析入口：\(self.sourceID)", level: .debug)
                semaphore.signal()
                return
            }
            if let handler {
                self.context.setObject(handler, forKeyedSubscript: "__atmusicHandler" as NSString)
            }
            self.context.setObject(payload as NSDictionary, forKeyedSubscript: "__atmusicPayload" as NSString)
            self.context.setObject(done, forKeyedSubscript: "__atmusicDone" as NSString)
            let invocation: String
            if handler != nil {
                invocation = "__atmusicHandler(__atmusicPayload)"
            } else {
                switch resolverKind {
                case "musicPluginGetMusicUrl":
                    invocation = "__atmusicMusicPlugin.getMusicUrl(__atmusicPayload.source, __atmusicPayload.info.musicInfo.musicId || __atmusicPayload.info.musicInfo.songmid || __atmusicPayload.info.musicInfo.id, __atmusicPayload.info.type)"
                case "pluginGetMusicUrl":
                    invocation = "__atmusicPlugin.getMusicUrl(__atmusicPayload.source, __atmusicPayload.info.musicInfo.musicId || __atmusicPayload.info.musicInfo.songmid || __atmusicPayload.info.musicInfo.id, __atmusicPayload.info.type)"
                default:
                    invocation = "__atmusicPlugin.musicUrl(__atmusicPayload.source, __atmusicPayload.info.musicInfo, __atmusicPayload.info.type)"
                }
            }
            ...
            (function() {
                try {
                    Promise.resolve(
                        ...
                        function(value) { __atmusicDone.resolve(value); },
                        function(error) { __atmusicDone.reject(error && error.message ? error.message : String(error)); }
                    );
                } catch (error) {
                    __atmusicDone.reject(error && error.message ? error.message : String(error));
                }
            })();
            """)
            if self.context.exception != nil {
                ATMusicLogger.shared.log("第三方脚本调用异常：\(self.sourceID)", level: .debug)
                semaphore.signal()
            }
        }
        ...
        if semaphore.wait(timeout: deadline) == .timedOut {
            ATMusicLogger.shared.log("第三方脚本调用超时：\(sourceID) \(Int(timeout))s", level: .debug)
        }
        resultLock.lock()
...
final class LXScriptSourceRunner {
    ...
    private let runtimeQueue = DispatchQueue(label: "AT Music.LXScriptSourceRunner")
    private var cache: [String: ATMusicLXScriptRuntime] = [:]
    ...
    private func runtime(for source: ThirdPartySource, script: String) -> ATMusicLXScriptRuntime? {
        let key = "\(source.id)|\(script.hashValue)"
        if let cached = cache[key] { return cached }
        guard let runtime = ATMusicLXScriptRuntime(source: source, script: script) else { return nil }
        cache[key] = runtime
        return runtime
    }
